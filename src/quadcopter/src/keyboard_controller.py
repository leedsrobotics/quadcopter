...
class KeyMapping(object):
	...
	SwitchMode       = QtCore.Qt.Key.Key_M
	SwitchCam        = QtCore.Qt.Key.Key_C
	FlatTrim	 = QtCore.Qt.Key.Key_P
	Recalibrate      = QtCore.Qt.Key.Key_O
	Left		 = QtCore.Qt.Key.Key_U
	Right	         = QtCore.Qt.Key.Key_I
	Home		 = QtCore.Qt.Key.Key_H
...
# We add a keyboard handler to the DroneVideoDisplay to react to keypresses
	def keyPressEvent(self, event):
		...
		if controller is not None and not event.isAutoRepeat():
			...
			if key == KeyMapping.Emergency:
			...
			elif key == KeyMapping.SwitchMode:
				...
				controller.video.imageL = controller.video.Ros2CV()
			elif key == KeyMapping.Right:
				controller.video.imageR = controller.video.Ros2CV()
			elif key == KeyMapping.Home:
				controller.ToggleHome()
			else:
